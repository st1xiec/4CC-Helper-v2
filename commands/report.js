...
module.exports.run = async (bot, message, args, data) => {
    ...
    if (!args[0]) return bot.sendErrEmbed(new global.MessageEmbed(), '💾 Укажите пользователя через @', message);
    ...
    message.react('✅');
    message.react('📮');
    message.author.send(`**Ваш репорт на ${user} успешно отправлен. Ожидайте если понадобится c вами свяжутся.**`);
    let reportsChannel = message.guild.channels.cache.get(data.Guild.reportChannel)
    if (!reportsChannel) {
        reportsChannel = await message.guild.channels.create(`👮‍reports-channel`, {
            type: 'text'
        })
    ...
    await reportsChannel.send(
        new global.MessageEmbed()
            ...
            .setThumbnail('https://media.giphy.com/media/7OVR6nwR6CTh2onJq2/giphy.gif')
    )
    message.delete()
...
module.exports.help = {
    ...
    usages: {
        'report @User#0001 Text.': 'Отправит жалобу на @User#0001 .',
