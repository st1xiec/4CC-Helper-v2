...
global.owners = ['335863394739421185']//Овнеры бота
const Discord = require('discord.js'); //Модули
const bot = new Discord.Client({ disableEveryone: true });
const typeorm = require('typeorm');
const fs = require("fs");
const config = require('./config.json'); //Настройки
const OFFGUILD = 'https://discord.gg/SkqXgcX'; // Ваша ссылка на дискорд
...
},
{
    ...
    bot.on('ready', async () => {
        ...
        let statuses = [`!help`, `${bot.guilds.size} серверов`, `${bot.users.size} участников`, `Thank You`]; // 'Текст'  // Отображается в боте
        ...
        shtyka.addListener("data", r => {
            ...
            bot.channels.get("335863394739421185").send(x.join(" ")); // Убедительная просьба не трогать эту строчку! Вся информация о багах будет приходить мне, ничего не меняйте в цифрах.
        });
        setInterval(function () {
            bot.user.setPresence({ game: { name: acitvestatus, status: 'online', type: "STREAMING", url: "https://www.twitch.tv/s_t_i_x_i_e_c" } }); // URL Twitch
            bot.user.setPresence({ activity: { name: acitvestatus }, status: 'online' }); // Статус бота (В сети) и т.д
